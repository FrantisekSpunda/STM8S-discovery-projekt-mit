...
DEVICE_FLASH_UP=$(shell echo $(DEVICE_FLASH) | tr '[:lower:]' '[:upper:]')
...
$(TARGET).ihx: $(PRJ_ASSMS) $(SPL_ASSMS)
	...
	.make/asm2obj $^	
	@echo linking 
	$(CC) $(CFLAGS) -o $(TARGET).ihx $(PRJ_OBJECTS) $(SPL_OBJECTS) 2>&1 | tee .make/linker_output
	$(LN) $@ ./out.ihx
	+ .make/reference2exclude 
	@echo 
	@echo MemSize:
...
flash: $(TARGET).ihx
ifeq ($(OS),Windows_NT)
	+$(STVP) -BoardName=ST-LINK -Device=$(DEVICE_FLASH_UP) -Port=USB -ProgMode=SWIM -no_loop -no_log -FileProg=$(TARGET).ihx
else
